...
export const generateInvoicePDF = (order: any) => {
    ...
    if (order.shipping_cost === 'Contested') {
        displayShipping = 'To be determined';
    } else if (!isNaN(shippingCost) && shippingCost > 0) {
        displayShipping = `${currency} ${shippingCost.toLocaleString()}`;
    } else if (!shippingCost && order.total_amount > subtotal) {
    ...
    // If we have VAT recorded
    if (vatRate > 0) {
        doc.text(`Includes VAT (${(vatRate * 100).toFixed(1)}%):`, 140, finalY + 16);
        doc.text(`${currency} ${vatAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 195, finalY + 16, { align: 'right' });
    } else {
        doc.text(`VAT (Export):`, 140, finalY + 16);
        doc.text(`${currency} 0.00`, 195, finalY + 16, { align: 'right' });
    }

    doc.text(`Shipping:`, 140, finalY + 22);
    doc.text(displayShipping, 195, finalY + 22, { align: 'right' });
